...
@torch.no_grad()
def evaluate(model, head_loader, tail_loader, args, train_type = 'valid'):
    model.eval()
    ...
    for i, (b1, b2) in enumerate(zip(head_loader, tail_loader)):
        for b in (b1, b2):
            ...
            score = model((positive_sample, negative_sample), mode, train_type = train_type)
...
def main():
    ...
    nentity = data.num_nodes_dict['disease']
    ...
    for i in range(1, args.max_step + 1):
        ...
        if i % 1000 == 0:
            valid_logs = evaluate(model, valid_dataloader_head, valid_dataloader_tail, args, 'valid')
            valid_metrics = {}
            ...
            test_logs = evaluate(model, test_dataloader_head, test_dataloader_tail, args, 'test')
            test_metrics = {}
