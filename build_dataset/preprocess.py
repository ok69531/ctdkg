...
def build_cg_graph(type, file_path = 'raw', save_path = 'processed/cg'):
    ...
    if type == 'v1':
        ...
        torch.save(data, f'{save_path}/cg-v1.pt')
        torch.save(chem_map, f'{save_path}/chem_map')
        ...
        torch.save(data, f'{save_path}/cg-v2.pt')
        torch.save(chem_map, f'{save_path}/chem_map')
        torch.save(gene_map, f'{save_path}/gene_map')
        torch.save(edge_type_map, f'{save_path}/rel_type_map')
...
def build_cgo_graph(file_path = 'raw', save_path = 'processed/cgo'):
    ...
    data.edge_index_dict = {
        ('chemical', 'chem_biological_go', 'gene_ontology'): torch.from_numpy(biological_chem_go.values.T).to(torch.long),
        ('chemical', 'chem_cellular_go', 'gene_ontology'): torch.from_numpy(cellular_chem_go.values.T).to(torch.long),
        ('chemical', 'chem_molecular_go', 'gene_ontology'): torch.from_numpy(molecular_chem_go.values.T).to(torch.long)
    }
    data.edge_reltype = {
        (h, r, t): torch.full((edge.size(1), 1), fill_value=edge_type_map[r]) for (h, r, t), edge in data.edge_index_dict.items()
    }
...
def build_dgo_graph(file_path = 'raw', save_path = 'processed/dgo'):
    print('>>> Processing GeneOntology-Disease Data ...')
    print('----------------------------------------------------------------------------')
    ...
    torch.save(data, f'{save_path}/dgo.pt')
    torch.save(pheno_map, f'{save_path}/go_map')
    torch.save(dis_map, f'{save_path}/dis_map')
    torch.save(edge_type_map, f'{save_path}/rel_type_map')

    print('GeneOntology-Disease graph is successfully constructed.')
    return data, save_path
...
def build_ctd_graph(file_path = 'processed', save_path = 'processed/ctd'):
    ...
    dgo_data = torch.load(f'{file_path}/dgo/dgo.pt')
    ...
    for (h, r, t) in tqdm(edge_index_dict.keys()):
        ...
        elif (h=='disease') & (t=='gene_ontology'):
            old_hmap = dgo_dis_map; old_tmap = dgo_go_map
            new_hmap = dis_map; new_tmap = go_map
...
# build_benchmarks('cpheno', 0.8, 0.1)
# build_benchmarks('cgo', 0.9, 0.05)
# build_benchmarks('gd', 0.98, 0.01)
# build_benchmarks('gpath', 0.8, 0.1)
# build_benchmarks('gpheno', 0.8, 0.1)
# build_benchmarks('ggo', 0.9, 0.05)
