...
class TestDataset(Dataset):
    ...
    def collate_fn(data):
        ...
        # data: [(pos, neg, mode), …]
        pos_batch = torch.stack([d[0] for d in data], dim=0)
        neg_list = [d[1] for d in data]                     # 각자 다른 길이
        mode = data[0][2]

        # 1) pad_sequence로 [B, L_max] 형태로 패딩
        neg_padded = pad_sequence(neg_list, batch_first=True, padding_value=1e+10)

        # 2) 실제 유효한 slot 표시할 mask 생성 (옵션)
        lengths = torch.tensor([n.size(0) for n in neg_list])
        mask = torch.arange(neg_padded.size(1))[None, :] < lengths[:, None]

        return pos_batch, neg_padded, mask, mode
