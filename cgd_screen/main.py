...
from dataset import LinkPredDataset, TrainDataset, TestDataset, BidirectionalOneShotIterator
...
args.dataset = 'cgd'
args.embedding_type = 'concat'
args.llm_model = 'biot5+'
args.test_log_steps = 600
...
wandb.init(project = f'cgd_screen', entity = 'soyoung')
...
@torch.no_grad()
def evaluate(model, tail_loader, args):
    model.eval()
    
    test_logs = defaultdict(list)
    for i, b in enumerate(tail_loader):
        positive_sample, negative_sample, negative_mask, mode = b
        positive_sample = positive_sample.to(device)
        
        for i in range(len(positive_sample)):
            one_positive_sample = positive_sample[i].unsqueeze(0)
            one_negative_sample = negative_sample[i][negative_mask[i]].unsqueeze(0).to(device)
            
            score = model((one_positive_sample, one_negative_sample), mode)
        ...
        if i % args.test_log_steps == 0:
            logging.info('Evaluating the model... (%d/%d)' % (i, int(len(tail_loader))))
...
def main():
    ...
    for i in range(1, args.max_step + 1):
        ...
        if i % 1000 == 0:
            valid_logs = evaluate(model, valid_dataloader_tail, args)
            valid_metrics = {}
            ...
            test_logs = evaluate(model, test_dataloader_tail, args)
            test_metrics = {}
