...
path_col = 'PathwayID'
go_col = 'GOID'
target_disease = 'Huntington Disease'
# target_disease = 'COVID-19'
# target_disease = 'Diabetes Mellitus'
...
def build_cd_graph(save_path = 'dataset/cd'):
    ...
    chem_dis_tmp = chem_dis_tmp[chem_dis_tmp['DiseaseName'] == target_disease]
    
    cd_chem = chem_dis_tmp.ChemicalID.unique()
    ...
    chem_dis_tmp = chem_dis_tmp[chem_dis_tmp.ChemicalID.isin(uniq_chem)]
...
def build_gd_graph(save_path = 'dataset/gd'):
    ...
    gene_dis_tmp = gene_dis_tmp[gene_dis_tmp['DiseaseName'] == target_disease]
    
    gd_gene = gene_dis_tmp.GeneID.unique()
    
    gene_desc_tmp = pd.read_csv(desc_path)
    gene_desc = gene_desc_tmp[gene_desc_tmp.GeneID.isin(gd_gene)]
    uniq_gene = gene_desc[gene_desc.SummarizedDescription.notna()].GeneID.unique()
    
    gene_dis_tmp = gene_dis_tmp[gene_dis_tmp.GeneID.isin(uniq_gene)]
...
def build_cg_graph(save_path = 'dataset/cg'):
    ...
    gene_map = torch.load('dataset/gd/gene_map')
    
    chem_idx = chem_gene_tmp.ChemicalID.isin(chem_map.keys())
    gene_idx = chem_gene_tmp.GeneID.isin(gene_map.keys())
    
    chem_gene_tmp = chem_gene_tmp[chem_idx & gene_idx]
...
def build_dpheno_graph(save_path = 'dataset/dpheno'):
    print('----------------------------------------------------------------------------')
    ...
    biological_pheno_dis_tmp = biological_pheno_dis_tmp[biological_pheno_dis_tmp.DiseaseName == target_disease]
    cellular_pheno_dis_tmp = cellular_pheno_dis_tmp[cellular_pheno_dis_tmp.DiseaseName == target_disease]
    molecular_pheno_dis_tmp = molecular_pheno_dis_tmp[molecular_pheno_dis_tmp.DiseaseName == target_disease]

    bio_pheno = biological_pheno_dis_tmp.GOID.unique()
    cell_pheno = cellular_pheno_dis_tmp.GOID.unique()
    mol_pheno = molecular_pheno_dis_tmp.GOID.unique()
    phenos = np.unique(np.union1d(np.union1d(bio_pheno, cell_pheno), mol_pheno))
    
    pheno_path = '../build_dataset/processed/description/phenotype_description.csv'
    pheno_desc_tmp = pd.read_csv(pheno_path)
    pheno_desc = pheno_desc_tmp[pheno_desc_tmp.PhenotypeID.isin(phenos)]
    uniq_pheno = pheno_desc[pheno_desc.SummarizedDescription.notna()].PhenotypeID.unique()
    
    biological_pheno_dis_tmp = biological_pheno_dis_tmp[biological_pheno_dis_tmp.GOID.isin(uniq_pheno)]
    cellular_pheno_dis_tmp = cellular_pheno_dis_tmp[cellular_pheno_dis_tmp.GOID.isin(uniq_pheno)]
    molecular_pheno_dis_tmp = molecular_pheno_dis_tmp[molecular_pheno_dis_tmp.GOID.isin(uniq_pheno)]
...
def build_cpheno_graph(save_path = 'dataset/cpheno'):
    print('----------------------------------------------------------------------------')
    ...
    chem_map = torch.load('dataset/cd/chem_map')
    pheno_map = torch.load('dataset/dpheno/pheno_map')
    
    chem_id = chem_pheno_tmp.chemicalid.isin(chem_map.keys())
    pheno_id = chem_pheno_tmp.phenotypeid.isin(pheno_map.keys())
    
    chem_pheno_tmp = chem_pheno_tmp[chem_id & pheno_id]
...
def build_gpheno_graph(save_path = 'dataset/gpheno'):
    print('----------------------------------------------------------------------------')
    print('>>> Processing Gene-Phenotype Data ...')
    
    file_path = '../build_dataset/raw/gene_phenotype'
    ...
    gene_id = gene_pheno_tmp[gene_col].isin(gene_map.keys())
    pheno_id = gene_pheno_tmp[pheno_col].isin(pheno_map.keys())
    
    gene_pheno_tmp = gene_pheno_tmp[gene_id & pheno_id]
...
def build_cpath_graph(save_path = 'dataset/cpath'):
    print('----------------------------------------------------------------------------')
    ...
    chem_map = torch.load('dataset/cd/chem_map')
    chem_idx = chem_path_tmp.ChemicalID.isin(chem_map.keys())
    chem_path_tmp = chem_path_tmp[chem_idx]
    
    desc_path = '../build_dataset/processed/description/pathway_description.csv'
    cpath_path = chem_path_tmp.PathwayID.unique()
    
    path_desc_tmp = pd.read_csv(desc_path)
    path_desc = path_desc_tmp[path_desc_tmp.PathwayID.isin(cpath_path)]
    uniq_path = path_desc[path_desc.SummarizedDescription.notna()].PathwayID.unique()
    
    chem_path_tmp = chem_path_tmp[chem_path_tmp.PathwayID.isin(uniq_path)]
...
def build_dpath_graph(save_path = 'dataset/dpath'):
    print('----------------------------------------------------------------------------')
    ...
    dis_path_tmp = dis_path_tmp[dis_path_tmp['# DiseaseName'] == target_disease]
    
    path_map = torch.load('dataset/cpath/path_map')
    path_idx = dis_path_tmp.PathwayID.isin(path_map.keys())
    dis_path_tmp = dis_path_tmp[path_idx]
...
def build_gpath_graph(save_path = 'dataset/gpath'):
    print('----------------------------------------------------------------------------')
    ...
    path_map = torch.load('dataset/dpath/path_map')
    
    gene_idx = gene_path_tmp.GeneID.isin(gene_map.keys())
    path_idx = gene_path_tmp.PathwayID.isin(path_map.keys())
    
    gene_path_tmp = gene_path_tmp[gene_idx * path_idx]
...
def build_cgo_graph(save_path = 'dataset/cgo'):
    print('----------------------------------------------------------------------------')
    ...
    chem_map = torch.load('dataset/cd/chem_map')
    chem_idx = chem_go_tmp.ChemicalID.isin(chem_map.keys())
    chem_go_tmp = chem_go_tmp[chem_idx]
    
    desc_path = '../build_dataset/processed/description/go_description.csv'
    cgo_go = chem_go_tmp.GOTermID.unique()
    
    go_desc_tmp = pd.read_csv(desc_path)
    go_desc = go_desc_tmp[go_desc_tmp.GOTermID.isin(cgo_go)]
    uniq_go = go_desc[go_desc.SummarizedDescription.notna()].GOTermID.unique()
    
    chem_go_tmp = chem_go_tmp[chem_go_tmp.GOTermID.isin(uniq_go)]
...
def build_ggo_graph(save_path = 'dataset/ggo'):
    print('----------------------------------------------------------------------------')
    ...
    gene_map = torch.load('dataset/gd/gene_map')
    go_map = torch.load('dataset/cgo/go_map')
    
    bio_gene_idx = biological_gene_go_tmp[gene_col].isin(gene_map.keys())
    bio_go_idx = biological_gene_go_tmp[go_col].isin(go_map.keys())
    cell_gene_idx = celluar_gene_go_tmp[gene_col].isin(gene_map.keys())
    cell_go_idx = celluar_gene_go_tmp[go_col].isin(go_map.keys())
    mol_gene_idx = molecular_gene_go_tmp[gene_col].isin(gene_map.keys())
    mol_go_idx = molecular_gene_go_tmp[go_col].isin(go_map.keys())
    
    biological_gene_go_tmp = biological_gene_go_tmp[bio_gene_idx & bio_go_idx]
    celluar_gene_go_tmp = celluar_gene_go_tmp[cell_gene_idx & cell_go_idx]
    molecular_gene_go_tmp = molecular_gene_go_tmp[mol_gene_idx & mol_go_idx]
...
def build_cgd_graph(file_path = 'dataset', save_path = 'dataset/cgd'):
    print('----------------------------------------------------------------------------')
...
def build_cgpd_graph(file_path = 'dataset', save_path = 'dataset/cgpd'):
    print('----------------------------------------------------------------------------')
...
def build_ctd_graph(file_path = 'dataset', save_path = 'dataset/huntington'):
    print('----------------------------------------------------------------------------')
